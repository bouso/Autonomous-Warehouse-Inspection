...
def parseKML(filename):
    ...
    print
    "Error: Didn't find a linestring in " + filename
    sys.exit(-3)
...
<trk>"""
    print
    "  <name><![CDATA[" + activity_type + " " + time_plain + "]]></name>"
    print
    "  <time>" + time_zulu + "</time>"
    print
    "  <trkseg>"
    for v in track:
        print
        "    <trkpt lat=\"{0}\" lon=\"{1}\"><time>{2}</time></trkpt>".format(v[1], v[0], v[2])
    print
    "  </trkseg>"
    print
    "</trk>"
...
<TrainingCenterDatabase xmlns="http://www.garmin.com/xmlschemas/TrainingCenterDatabase/v2" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.garmin.com/xmlschemas/TrainingCenterDatabase/v2 http://www.garmin.com/xmlschemas/TrainingCenterDatabasev2.xsd">"""
    ...
    "<Activities>"
    print
    "  <Activity Sport=\"" + activity_type + "\">"
    print
    "    <Id>" + time_zulu + "</Id>"
    print
    "      <Lap StartTime=\"" + time_zulu + "\">"
    ...
    for v in track:
        ...
        "      <Trackpoint>"
        print
        "        <Time>{0}</Time>".format(v[2])
        print
        "        <Position>"
        print
        "           <LatitudeDegrees>{0}</LatitudeDegrees>".format(v[1])
        print
        "           <LongitudeDegrees>{0}</LongitudeDegrees>".format(v[0])
        print
        "        </Position>"
        print
        ...
        print
        "        <HeartRateBpm><Value>" + str(heart_rate) + "</Value></HeartRateBpm>"
        print
        "      </Trackpoint>"
    print
    "       </Track>"
...
def dumpHelp():
    ...
    "runfaker.py <options>"
    print
    "  -i input_template.kml  : kml file to use as a template for track"
    print
    "  -o output.gpx          : output filename for gpx track"
    print
    "  -d date                : starting date for track (2014-10-26T11:00)"
    print
    "  -m minutes             : how many minutes the track should go on for"
    print
    "  -s mph_speed           : how fast you should go"
    sys.exit(2)
